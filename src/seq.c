...
static int read_sequence_from_fastq_from_stream(FILE * fp, Sequence * seq, int max_read_length)
{
    ...
    // Slightly different behaviour to reading from file. Will be the same if no tabs in ID string, but if there are, these won't get through in stream version
...
void alloc_sequence(Sequence * seq, int max_read_length, int max_name_length, char offset)
{
	...
	seq->id_string = malloc(sizeof(char) * max_name_length);
	if (seq->id_string == NULL) {
		fputs("Out of memory trying to allocate string\n", stderr);
		exit(1);
	}
...
char* sequence_get_quality_string(Sequence * seq, char* string)
{
    int i;
    
    for (i=0; i<seq->length; i++) {
        string[i] = seq->qual[i] + seq->qual_offset;
    }
